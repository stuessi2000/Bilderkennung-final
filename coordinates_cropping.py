...
docx_path = 'cubes_coordinate/koord.system Kopie.docx'
...
# Funktion zum Lesen der Koordinaten aus der .docx-Datei
def read_coordinates_from_docx(docx_path):
    print(f"Versuche, Koordinaten aus {docx_path} zu lesen...")
    try:
        doc = Document(docx_path)
        print(f"Dokument geladen. Anzahl der Tabellen: {len(doc.tables)}")
        coordinates = []
        
        # Versuche, Koordinaten aus Tabellen zu lesen
        if len(doc.tables) > 0:
            for i, table in enumerate(doc.tables):
                print(f"Tabelle {i+1}: {len(table.rows)} Zeilen, {len(table.columns) if len(table.rows) > 0 else 0} Spalten")
                
                for j, row in enumerate(table.rows):
                    if j == 0:  # Überschriftenzeile überspringen
                        print(f"Überschriften: {[cell.text for cell in row.cells]}")
                        continue
                        
                    try:
                        # Ausgabe der Zellenwerte für Debugging
                        cell_values = [cell.text.strip() for cell in row.cells]
                        print(f"Zeile {j}: {cell_values}")
                        
                        # Gehe davon aus, dass die Koordinaten in den Spalten X, Y, RX, RY, RZ sind
                        if len(row.cells) >= 6:  # Sicherstellen, dass genügend Zellen vorhanden sind
                            x = float(row.cells[1].text.strip())  # X-Koordinate
                            y = float(row.cells[2].text.strip())  # Y-Koordinate
                            rx = float(row.cells[3].text.strip())  # RX-Rotation
                            ry = float(row.cells[4].text.strip())  # RY-Rotation
                            rz = float(row.cells[5].text.strip())  # RZ-Rotation
                            coordinates.append((x, y, rx, ry, rz))
                        else:
                            print(f"Zeile {j} hat nicht genügend Zellen: {len(row.cells)}")
                    except Exception as e:
                        print(f"Fehler beim Verarbeiten der Zeile {j}: {e}")
        else:
            # Wenn keine Tabellen vorhanden sind, versuche, Koordinaten aus dem Text zu extrahieren
            print("Keine Tabellen gefunden. Versuche, Koordinaten aus dem Text zu extrahieren...")
            current_object_coords = {}
            object_identifier = "Initial" # Für Debugging

            for para_idx, para in enumerate(doc.paragraphs):
                text = para.text.strip()
                if text:
                    print(f"Para {para_idx} ({object_identifier if current_object_coords else 'Pre-Objekt'}): {text[:100]}...")
                    
                    if text.startswith("Objekt"):
                        if 'X' in current_object_coords and 'Y' in current_object_coords and 'RZ' in current_object_coords:
                            print(f"  ({object_identifier}) Speichere vorheriges Objekt: {current_object_coords}")
                            coordinates.append((
                                current_object_coords['X'], 
                                current_object_coords['Y'], 
                                current_object_coords['RZ']
                            ))
                        
                        obj_match = re.search(r"Objekt\s*(\S+)", text)
                        object_identifier = obj_match.group(1) if obj_match else f"UnbekanntesObjekt_Para{para_idx}"
                        current_object_coords = {}
                        print(f"  Neues Objekt '{object_identifier}' gestartet.")

                    match_x = re.search(r'\bX-Wert:\s*(-?\d+\.?\d*)', text)
                    if match_x:
                        val_x = float(match_x.group(1))
                        current_object_coords['X'] = val_x
                        print(f"    ({object_identifier}) Gefunden X: {val_x}")
                        
                    match_y = re.search(r'\bY-Wert:\s*(-?\d+\.?\d*)', text)
                    if match_y:
                        val_y = float(match_y.group(1))
                        current_object_coords['Y'] = val_y
                        print(f"    ({object_identifier}) Gefunden Y: {val_y}")
                        
                    match_rz = re.search(r'\bRZ-Wert:\s*(-?\d+\.?\d*)', text) # Auch hier zur Konsistenz \b hinzugefügt
                    if match_rz:
                        val_rz = float(match_rz.group(1))
                        current_object_coords['RZ'] = val_rz
                        print(f"    ({object_identifier}) Gefunden RZ: {val_rz}")
            
            if 'X' in current_object_coords and 'Y' in current_object_coords and 'RZ' in current_object_coords:
                print(f"  ({object_identifier}) Speichere letztes Objekt nach Schleife: {current_object_coords}")
                coordinates.append((
                    current_object_coords['X'], 
                    current_object_coords['Y'], 
                    current_object_coords['RZ']
        ...
        # Wenn keine Koordinaten gefunden wurden, verwende Standardkoordinaten für die Würfel
        if not coordinates:
            print("Keine Koordinaten gefunden. Verwende Standardkoordinaten für die Würfel.")
            # Für jeden Würfel im individual_cubes Ordner eine Standardkoordinate erstellen
            cube_files = list(Path(image_folder).glob('*.png'))
            for i, _ in enumerate(cube_files):
                # Standardkoordinaten: Mitte des Bildes, keine Rotation
                coordinates.append((100, 100, 0, 0, 0))
        
        logging.info(f"Gelesene Koordinaten aus DOCX: {coordinates}")
        print(f"Insgesamt {len(coordinates)} Koordinaten aus DOCX-Datei gefunden.")
        return coordinates
    except Exception as e:
        print(f"Fehler beim Lesen der DOCX-Datei {docx_path}: {e}")
        return []
...
def process_images():
    ...
    # Suche nach Koordinatendateien (.txt hat höhere Priorität als .docx)
    ...
    # Sicherstellen, dass wir genug Koordinaten für alle Bilder haben (oder zumindest eine, wenn keine DOCX-Koordinaten gelesen wurden)
    if not coordinates and image_files:
        print("Keine Koordinaten aus DOCX gelesen, verwende Standard für Debugging, falls Bilder vorhanden.")
    ...
    # KPI-Ausgabe am Ende
    if cubes_saved > 0:
        print(f"→ {cubes_saved} Faces aus {source_image_stem}.png extrahiert\n")
    else:
        print(f"— Keine Faces gefunden in {source_image_stem}.png\n")
...
def save_cube_centers_csv(cube_centers_list, csv_filename="cubes_centers.csv"):
    """
    Speichert die Mittelpunkte aller Würfel in einer CSV-Datei
    Format: FaceImage, Center_X_px, Center_Y_px
    """
    import csv
    
    print(f"Speichere {len(cube_centers_list)} Würfel-Mittelpunkte in {csv_filename}")
    
    with open(csv_filename, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["FaceImage", "Center_X_px", "Center_Y_px"])
        writer.writeheader()
        for row in cube_centers_list:
            writer.writerow(row)
    
    print(f"✓ Mittelpunkte aller Würfel-Faces gespeichert in {csv_filename}")
